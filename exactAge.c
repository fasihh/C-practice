...
int isValid(int date, int month, int year) {
	if (date > 31 || month > 12 || month < 0 || year < 0) {return 0;};
	if ((month == 1 || month == 3 || month == 5 || month == 7 || month == 8 || month == 10 || month == 12) && date > 31) {return 0;};
	if ((month == 4 || month == 6 || month == 9 || month == 11) && date > 30) {return 0;};

	int isLeap = year % 4 == 0 || year % 400 == 0 && year % 100 != 0;

	if (!isLeap && date > 28 && month == 2) {
		return 0;
	};

	if (isLeap && date > 29 && month == 2) {
		return 0;
	};
	return 1;
};
